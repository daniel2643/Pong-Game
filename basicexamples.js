...
function dragRectObservable() {
    ...
    mousemove = Observable.fromEvent(svg, 'mousemove'), 
    ...
    mouseDownInRect = rect.observe('mousedown');
    ...
    mouseDownOffset = mouseDownInRect.map(({ clientX, clientY }) => ({ xOffset: Number(rect.attr('x')) - clientX,
        yOffset: Number(rect.attr('y')) - clientY }));
    ...
    trackingDraggedRect.map(({ x, y }) => ({ x: x, y: y })).subscribe(({ x, y }) => rect.attr('x', x)
        .attr('y', y));
}
/**
 * An example of traditional event driven programming style - this is what we are
...
function drawRectsObservable() {
    ...
    mousedown.subscribe(({ clientX, clientY }) => {
        const svgRect = svg.getBoundingClientRect(), x0 = clientX - svgRect.left, y0 = clientY - svgRect.top, rect = new Elem(svg, 'rect')
            .attr('x', clientX - svgRect.left).attr('y', clientY - svgRect.top) // initial rect's coordinate 
            .attr('width', 5).attr('height', 5)
            .attr('fill', '#11B3D7');
        mousemove
            ...
            // set final position of rect
            .subscribe(({ x, y, width, height }) => rect.attr('x', x).attr('y', y).attr('width', width).attr('height', height));
    });
}
function drawRectsObservable2(svg) {
    ...
    return mousedown.map(({ clientX, clientY }) => {
        ...
        mousemove
            ...
            rect.attr('x', Math.min(x0, clientX - svgRect.left))
...
function dragRectObservable2(svg, rect) {
    const 
    //svg is the targeted panel in html page, where we want the mouse events to be monitiored 
    //An observable from traking mouse moves
    mousemove = Observable.fromEvent(svg, 'mousemove'), 
    ...
    const 
    //An observable to track mousedown events in the rect we've given 
    mouseDownInRect = rect.observe('mousedown').map(e => { e.stopPropagation(); return e; }); // other observer don't have chance to observe MouseEvent
