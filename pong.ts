...
function pong() : void {
  ...
  const 
    svg = document.getElementById("canvas")!,  // get canvas svg
  ...
  let 
    leftPlayerScore = 0,
    rightPlayerScore = 0,
    leftWin = "Game Over",   // the label to display on page when left player wins
  ...
  let 
    ...
    gridPos : Elem[] = new Array(Math.floor((HEIGHT-10)/15)+1).fill(getRect(svg, 0, 0, 0, 0))
  ...
  const 
    leftPaddle = getRect(svg, 0, HEIGHT/2-PADDLEHEIGHT/2, PADDLEWIDTH, PADDLEHEIGHT, '#13B443'),
    rightPaddle = getRect(svg, WIDTH-PADDLEWIDTH, HEIGHT/2-PADDLEHEIGHT/2, PADDLEWIDTH, PADDLEHEIGHT, '#13B443'),
    ball = getCircle(svg, -10, Math.random()*HEIGHT, RADIUS)
  ...
  let 
    xstep = 1,  // the initial x-coordinate steps of ball in a unit time period. i.e. speed of ball in x-coordinate
  ...
  Observable.interval(0)
    .takeUntil(Observable.interval(0).filter(num=>{
      const 
        judge = leftPlayerScore>=WINSCORE || rightPlayerScore>=WINSCORE;  // judgement for stoping the Observable
      judge ? [ball.attr('cx', -1000)] : {}
      return judge
    }))
    .subscribe(()=> {
      const 
        cx = Number(ball.attr('cx')), 
        cy = Number(ball.attr('cy')),
        ball_o = Observable.interval(0).map(num=>ball),  //create an Observable<ball> stream
  ...
  Observable.fromEvent<KeyboardEvent>(document, 'keydown').subscribe(e=> {
    switch(e.key){
      case 'q': keys[81] = true; break;   // 'Q': leftpaddle up
      case 'a': keys[65] = true; break;   // 'A': leftpaddle down
      case 'p': keys[80] = true; break;   // 'P': rightpaddle up
      case 'l': keys[76] = true; break;   // 'L': rightpaddle down

    }
  })

  // Observable to make pressed key false when press up any key of keyboard
  Observable.fromEvent<KeyboardEvent>(document, 'keyup').subscribe(e=> {
    switch(e.key){
      case 'q': keys[81] = false; break;   // 'Q'
      case 'a': keys[65] = false; break;   // 'A'
      case 'p': keys[80] = false; break;   // 'P'
      case 'l': keys[76] = false; break;   // 'L'
    ...
    if (!keys[83] && !keys[68] && !keys[77]){
      switch(e.key){
        case 's':   // 's'(single): Mode 1: single player by keyboard control
          ...
          paddleMove<Elem>(rightPaddle, 80, 76)                                 
          deleteInstruction()
          break;  
        case 'd':   // 'd'(double): Mode 2: double player by keyboard control
          ...
          keys[68] = true; 
          // right paddle move by human
          paddleMove<Elem>(leftPaddle, 81, 65)  
          // right paddle move by human                                                   
          paddleMove<Elem>(rightPaddle, 80, 76) 
          deleteInstruction()
        ...
        case 'm':   // 'm'(single by mouse): Mode 3: single player by mouse control 
          ...
          paddleMoveByMouse<Elem>(svg, rightPaddle)                            
          deleteInstruction() 
          break; 
      }
    }  
  ...
  const 
    score = document.getElementById("score")!,   // get the score object from HTML
...
function getRect(target:HTMLElement, x:number, y:number, width:number, height:number, color:string='#11B3D7') : Elem {
  /**
...
function getCircle(target:HTMLElement, cx:number, cy:number, radius:number, color:string='#FFA500') : Elem{
  /**
...
function paddleMove<T extends Elem>(paddle:T, upKeyCode: number, downKeyCode: number) : void{
  /**
  ...
  Observable
    ...
    .subscribe(e=>{
      ...
      Observable.interval(0)
        ...
        .subscribe(()=>{
          const 
            currentY = Number(paddle.attr('y')),   // the paddle's y-coordinate
...
function paddleMoveByMouse<T extends Elem>(table:HTMLElement, paddle:T) :void{
  /**
   * Simulating a paddleMove action by human-controlled mouse event
   */
  const
    mousemove = Observable.fromEvent<MouseEvent>(table, 'mousemove'),
    mouseup = Observable.fromEvent<MouseEvent>(table, 'mouseup'),
    mousedown = Observable.fromEvent<MouseEvent>(table, 'mousedown');
...
function deleteInstruction(){
   ...
   */
  const 
  instruction = document.getElementById("instruction")!   // get the introduction message object from HTML
...
let 
  keys : boolean[] = new Array(300).fill(false);   
